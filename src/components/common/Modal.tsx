...
export const Modal: React.FC<ModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/60 dark:bg-slate-950/80 backdrop-blur-sm animate-fadeIn">
      <div
        className={`bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl w-full ${widthStyles[maxWidth]} shadow-2xl overflow-hidden flex flex-col max-h-[90vh] transition-colors`}
      >
        ...
        <div className="p-6 overflow-y-auto text-slate-900 dark:text-slate-100">{children}</div>
      </div>
    </div>
  );
};
