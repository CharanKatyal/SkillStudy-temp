...
export const PortfolioCustomizer: React.FC<PortfolioCustomizerProps> = ({ isOpen, onClose }) => {
  const { portfolio, updatePortfolio, managedProjects, ideProjects } = useData();
      ...
      <div className="space-y-4">
        ...
        <div>
          ...
          <textarea
            ...
            rows={3}
            className="w-full bg-slate-950 border border-slate-800 rounded-xl px-3.5 py-2 text-xs text-slate-100 focus:outline-none resize-none"
          />
        </div>
        ...
        {/* Skills Tag Editor */}
        <div>
          <label className="block text-xs font-semibold text-slate-300 mb-1">Portfolio Skills</label>
          <div className="flex flex-wrap gap-2 p-2 bg-slate-950 rounded-xl border border-slate-800 mb-2">
            {skills.map((s, idx) => (
              <span
                key={idx}
          ...
          <form onSubmit={handleAddSkill} className="flex gap-2">
            <input
              ...
              className="flex-1 bg-slate-950 border border-slate-800 rounded-lg px-3 py-1.5 text-xs text-slate-100 focus:outline-none"
            />
            <select
              value={newSkillLevel}
              onChange={e => setNewSkillLevel(e.target.value as any)}
              className="bg-slate-950 border border-slate-800 rounded-lg px-2 text-xs text-slate-200"
            >
