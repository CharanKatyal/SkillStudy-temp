      ...
      <div className="p-6 rounded-2xl bg-gradient-to-r from-slate-850 to-slate-900 border border-slate-750 flex flex-col md:flex-row md:items-center justify-between gap-6 shadow-lg">
        <div className="space-y-2">
          <div className="flex items-center gap-2">
            <span className="flex items-center gap-1.5 px-3 py-1 rounded-full bg-amber-950/70 border border-amber-800/60 text-amber-300 text-xs font-bold">
              <Flame className="w-4 h-4 text-orange-400" />
              <span>{progress?.streak?.current || 1} Day Streak (Best: {progress?.streak?.longest || 1})</span>
            </span>
          </div>
          <h2 className="text-2xl font-extrabold text-slate-100">Achievements &amp; Honors</h2>
        ...
        <div className="w-full md:w-64 bg-slate-900 p-4 rounded-xl border border-slate-800 shrink-0">
          <div className="flex justify-between text-xs text-slate-400 mb-1.5">
            ...
            <span className="font-bold text-slate-200">{unlockedCount} / {totalCount}</span>
          </div>
          <ProgressBar value={pct} color="bg-amber-500" height="md" />
          <p className="text-[11px] text-amber-400 font-semibold mt-2">{pct}% Completed</p>
        </div>
      </div>
        ...
        {filtered.map(ach => (
          <Card
            ...
            className={`p-5 flex flex-col justify-between border transition ${
              ach.unlocked
                ? 'bg-gradient-to-br from-slate-850 to-amber-950/20 border-amber-700/50 shadow-md shadow-amber-950/20'
                : 'bg-slate-850/60 border-slate-800 opacity-60'
            }`}
          >
            <div>
                ...
                <div
                  className={`w-12 h-12 rounded-2xl flex items-center justify-center border shrink-0 ${
                    ach.unlocked
                      ...
                      : 'bg-slate-800 text-slate-600 border-slate-750'
                  }`}
                >
