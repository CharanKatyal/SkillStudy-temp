...
export const DashboardView: React.FC = () => {
  ...
  return (
    <div className="space-y-8 animate-fadeIn">
      {/* 1. Welcome & Live Timetable Focus Banner */}
      <div className="relative overflow-hidden rounded-3xl bg-gradient-to-r from-brand-50 via-white to-emerald-50/60 dark:from-brand-900/40 dark:via-brand-950/20 dark:to-slate-900 border border-brand-200 dark:border-brand-500/20 p-6 sm:p-8 shadow-sm dark:shadow-md">
        <div className="relative z-10 flex flex-col lg:flex-row lg:items-center justify-between gap-6">
          <div className="space-y-2">
            <div className="flex flex-wrap items-center gap-2">
              <div className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-brand-500/10 border border-brand-500/20 text-brand-700 dark:text-brand-400 text-xs font-bold">
                <Clock className="w-3.5 h-3.5 animate-pulse" />
                <span>{scheduleStatus.currentDay} • {scheduleService.minutesTo12Hour(nowMinutes)}</span>
              </div>

              {scheduleStatus.isSchoolTime ? (
                <span className="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full bg-sky-100 dark:bg-sky-950/60 text-sky-800 dark:text-sky-300 text-xs font-bold">
            ...
            <p className="text-slate-600 dark:text-slate-300 text-sm max-w-xl leading-relaxed">
              {scheduleStatus.subtitle} {scheduleStatus.formattedRemainingTime ? `(${scheduleStatus.formattedRemainingTime} remaining)` : ''}
            </p>
          </div>

          <div className="flex flex-wrap items-center gap-3 shrink-0">
            {scheduleStatus.activeSlot?.type === 'coding' ? (
              <button
            ...
            ) : scheduleStatus.activeSlot?.type === 'project' ? (
              <button
                onClick={() => setActiveNav('projects')}
                className="px-5 py-2.5 rounded-xl bg-pink-600 hover:bg-pink-500 text-white font-bold text-xs flex items-center gap-2 shadow-md transition"
              >
                <FolderKanban className="w-4 h-4" />
                <span>Open Projects</span>
              </button>
            ) : scheduleStatus.activeSlot?.type === 'study' || scheduleStatus.activeSlot?.type === 'revision' ? (
              <button
            ...
            <button
              onClick={() => setActiveNav('planner')}
              className="px-4 py-2.5 rounded-xl bg-white dark:bg-slate-850 hover:bg-slate-100 dark:hover:bg-slate-800 text-slate-700 dark:text-slate-300 font-bold text-xs border border-slate-200 dark:border-slate-750 transition"
            >
        ...
        {scheduleStatus.progressPercent > 0 && (
          <div className="mt-4 pt-3 border-t border-brand-200/50 dark:border-brand-500/10">
            <div className="flex justify-between text-xs text-slate-600 dark:text-slate-400 mb-1 font-semibold">
              ...
              <span>{scheduleStatus.progressPercent}% Complete ({scheduleStatus.formattedRemainingTime} left)</span>
            </div>
      ...
      {/* 2. Today's Visual Schedule Strip */}
      <Card className="p-5">
        <div className="flex items-center justify-between mb-3">
          <div className="flex items-center gap-2 text-xs font-bold uppercase tracking-wider text-slate-600 dark:text-slate-400">
            ...
            <span>Today's Routine Timeline ({scheduleStatus.todaySlots.length} Blocks)</span>
          </div>
          <button
            onClick={() => setActiveNav('planner')}
        ...
        <div className="flex items-center gap-2.5 overflow-x-auto pb-2 scrollbar-none">
          ...
          {schedule?.schoolHours.enabled && schedule.schoolHours.days.includes(scheduleStatus.currentDay) && (
            <div
              className={`p-3 rounded-xl border shrink-0 min-w-[170px] ${
                scheduleStatus.isSchoolTime
                  ? 'bg-sky-500 text-white border-sky-600 shadow-md ring-2 ring-sky-300 dark:ring-sky-900'
            ...
            >
              <div className="flex items-center justify-between text-[11px] mb-1 font-bold">
                <span>{schedule.schoolHours.name || 'School Hours'}</span>
                {scheduleStatus.isSchoolTime && <span className="text-[9px] bg-white/30 px-1 rounded">NOW</span>}
              </div>
          ...
          {schedule?.mode === 'open' ? (
            ...
            scheduleStatus.todaySlots.map(slot => {
              ...
              return (
                <div
                  key={slot.id}
                  className={`p-3 rounded-xl border shrink-0 min-w-[180px] transition ${
                    isCurrent
                      ? 'bg-brand-600 text-white border-brand-700 shadow-md ring-2 ring-brand-300 dark:ring-brand-900'
                      : isPast
                      ? 'bg-slate-50 dark:bg-slate-850/50 border-slate-200 dark:border-slate-800 opacity-60 text-slate-700 dark:text-slate-300'
                      : 'bg-white dark:bg-slate-850 border-slate-200 dark:border-slate-800 text-slate-800 dark:text-slate-200'
                  }`}
                  style={!isCurrent ? { borderLeftWidth: '4px', borderLeftColor: slot.color || '#3b82f6' } : {}}
                >
                  <div className="flex items-center justify-between text-xs font-bold mb-0.5 truncate">
                    <span className="truncate mr-1">{slot.title}</span>
                    {isCurrent ? (
                      <span className="text-[9px] bg-white/30 px-1 rounded font-extrabold shrink-0">
                        NOW
                      </span>
                    ) : isPast ? (
